...
brew-install() {
  ...
  for package in "$@"; do
    ...
    if [ -z "${package_version}" ]; then
      ...
      echo -e "Homebrew package ${COLORS__BLUE}${package}${COLORS__CLEAR} already installed"
    fi
  done
}
...
npm-install() {
  ...
  for package in "$@"; do
    ...
    if [ "$?" -ne "0" ]; then
      ...
      echo -e "NPM package ${COLORS__BLUE}${package}${COLORS__CLEAR} already installed"
    fi
  done
}
...
brew-cask-install-dialog() {
  ...
  for package in ${choices}; do
    ...
    if [ -z "${package_version}" ]; then
      ...
      echo -e "Homebrew cask package ${COLORS__BLUE}${package}${COLORS__CLEAR} already installed"
    fi
  done
}
